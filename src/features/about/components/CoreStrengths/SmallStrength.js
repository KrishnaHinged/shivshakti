...
/**
 * SmallStrength displays the smaller/compact cards in the strengths grid.
 */
export function SmallStrength({ strength, delay = 0 }) {
    ...
    >
      ...
      <div className="flex flex-col gap-1">
        <h4 className="text-xs font-bold text-slate-900 leading-tight relative inline-block pb-1">
          {strength.title}
          <span className={`absolute bottom-0 left-0 w-0 h-[2px] transition-all duration-200 group-hover:w-6 ${
